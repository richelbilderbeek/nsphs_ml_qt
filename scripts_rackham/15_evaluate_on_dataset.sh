...
for epoch in $(seq "${save_interval}" "${save_interval}" "${epochs}"); do
  ...
  csv_filenames=$(find . | grep -E "f1_score_[[:digit:]]+.*csv")
  # shellcheck disable=SC2046,SC2086 # word splitting is intended for the variable 'csv_filenames', hence the plural variable name :-)
  rename "s/.csv/_${epoch}.csv/s" $csv_filenames
done
