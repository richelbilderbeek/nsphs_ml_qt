...
#SBATCH --ntasks 1
#SBATCH --mem=16G
#SBATCH --job-name=20_start_issue_29
#SBATCH --output=20_start_issue_29.log
...
do
  ...
  gcae_experiment_params_filename=/proj/sens2021565/nobackup/nsphs_ml_qt_results/data_issue_29_$window_kb/experiment_params.csv
  ...
  jobid_21=$(sbatch -A sens2021565                                              --output=21_create_"${unique_id}"_params.log ./nsphs_ml_qt/scripts_bianca/21_create_issue_29_params.sh              "$gcae_experiment_params_filename" | cut -d ' ' -f 4)
