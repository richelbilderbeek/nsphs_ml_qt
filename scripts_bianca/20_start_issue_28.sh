...
#SBATCH --ntasks 1
#SBATCH --mem=16G
#SBATCH --job-name=20_start_issue_28
#SBATCH --output=20_start_issue_28.log
...
do
  ...
  jobid_21=$(sbatch -A sens2021565                                              --output=21_create_"${unique_id}"_params.log            ./nsphs_ml_qt/scripts_bianca/21_create_issue_28_params.sh   "$gcae_experiment_params_filename" | cut -d ' ' -f 4)
  jobid_22=$(sbatch -A sens2021565 --dependency=afterok:"$jobid_21"             --output=22_create_"${unique_id}"_data.log              ./nsphs_ml_qt/scripts_bianca/22_create_issue_28_data.sh     "$gcae_experiment_params_filename" | cut -d ' ' -f 4)
