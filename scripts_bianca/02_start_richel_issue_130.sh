...
#SBATCH -C usage_mail
#SBATCH --mem=16G
#SBATCH --job-name=02_start_richel_issue_130
#SBATCH --output=02_start_richel_issue_130.log
...
jobid_16=$(sbatch -A sens2021565 --dependency=afterok:$jobid_15 --output=16_analyse_${unique_id}.log  ./nsphs_ml_qt/scripts_bianca/16_create_tidy_results.sh                                                                | cut -d ' ' -f 4)
jobid_17=$(sbatch -A sens2021565 --dependency=afterok:$jobid_16 --output=17_zip_${unique_id}.log      ./nsphs_ml_qt/scripts_bianca/17_zip_depersonalized_data.sh $datadir $data $trainedmodeldir $superpops $metrics $epoch | cut -d ' ' -f 4)
